...
export function DifficultySelect({ onStart }: Props) {
  ...
  function handleCustomStart() {
    ...
    if (!rows || !cols || !bombs || rows < 2 || cols < 2 || bombs < 1) {
      setError('Rows and cols must be ≥ 2, bombs ≥ 1.');
      return;
    }
    if (bombs >= rows * cols) {
      setError('Bombs must be less than total cells.');
      return;
    }
  ...
  return (
    <div className="difficulty-select">
      <h1 className="difficulty-select__title">Bomb Buster</h1>
      <p className="difficulty-select__subtitle">Choose a difficulty to start</p>
      <div className="difficulty-select__buttons">
        <button className="difficulty-select__btn" onClick={() => handlePreset('beginner')}>
          <span className="difficulty-select__btn-name">Beginner</span>
          <span className="difficulty-select__btn-desc">9×9 · 10 bombs</span>
        </button>
        <button className="difficulty-select__btn" onClick={() => handlePreset('intermediate')}>
          <span className="difficulty-select__btn-name">Intermediate</span>
          <span className="difficulty-select__btn-desc">16×16 · 40 bombs</span>
        </button>
        <button className="difficulty-select__btn" onClick={() => handlePreset('expert')}>
          <span className="difficulty-select__btn-name">Expert</span>
          <span className="difficulty-select__btn-desc">30×16 · 99 bombs</span>
        </button>
        <button
          className="difficulty-select__btn difficulty-select__btn--custom"
          onClick={() => setShowCustom(s => !s)}
        >
          <span className="difficulty-select__btn-name">Custom</span>
          <span className="difficulty-select__btn-desc">Set your own size</span>
        </button>
      </div>
      {showCustom && (
